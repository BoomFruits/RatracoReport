...
{
    public class ReportBKTTDH03Handler : BaseReportService<ReportBKTTDH03Handler>, IService, IReportHandler<ReportBKTTDH03Input, ReportBKTTDH03Output>
    {
        public ReportBKTTDH03Handler(ICacheService cacheService) : base(cacheService)
        {
        }

        public async Task<ReportBKTTDH03Output> Handle(ReportBKTTDH03Input request)
        {
            //// Process don vi
            ...
            return new ReportBKTTDH03Output(request)
            {
        ...
        public async Task<byte[]> HandleExcel(ReportBKTTDH03Input request)
        {
            var report = await Handle(request);
            return ExportExcelTemplateAsBuffering("BKTTDH03.xlsx", (package) =>
            {
        ...
        private void BinOneRowExcel(ExcelRow cols, ReportObject item, int index, bool isTong)
        {
            ...
            cols["H"].UseValue(item.Get("PhanLoaiXe")).UseAlignLeft();
            ...
            cols["L"].UseValue(isTong ? "" : item.Get("TrongLuong")).UseAlignRight(); 
            cols["M"].UseValue(item.Get("DonViTinh")).UseAlignLeft(); 
            cols["N"].UseValue(item.Get("ThanhTien")).UseAlignRight(); 
        }

        public async Task<IEnumerable<ReportObject>> ExecuteSP(ReportBKTTDH03Input input)
        {
            using (var conn = await Connection.OpenConnectionReportAsync())
            {
                var sp = @"[dbo].[BangKe_BKTTDH03]";
    ...
    public class ReportBKTTDH03Input : IRequest<ReportBKTTDH03Output>
    {
    ...
    public class ReportBKTTDH03Output : BaseReportOutput<ReportBKTTDH03Input>
    {
        public IEnumerable<ReportObject> Data { get; set; }
        public ExpandoObject Tong { get; set; }
        public ReportBKTTDH03Output(ReportBKTTDH03Input input) : base(input)
        {
        }
    }
}
