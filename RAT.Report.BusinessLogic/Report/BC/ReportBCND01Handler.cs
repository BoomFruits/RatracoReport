...
{
    public class ReportBCND01Handler : BaseReportService<ReportBCND01Handler>, IService, IReportHandler<ReportBCND01Input, ReportBCND01Output>
    {
        public ReportBCND01Handler(ICacheService cacheService) : base(cacheService)
        {
        }

        public async Task<ReportBCND01Output> Handle(ReportBCND01Input request)
        {
            ReportBCND01Output result = new ReportBCND01Output(request);
            ...
            if (detail != null && detail.Any())
            {
                ...
                if (grp != null && grp.Any())
                {
                    var lst = new List<ReportBCND01Item>();
                    foreach (var d in grp)
                    {
                        var item = new ReportBCND01Item();
        ...
        public async Task<byte[]> HandleExcel(ReportBCND01Input request)
        {
            var report = await Handle(request);
            return ExportExcelTemplateAsBuffering("BCND01.xlsx", (package) =>
            {
        ...
        public async Task<IEnumerable<ReportBCND01ChiTietItem>> ExecuteSP(ReportBCND01Input input)
        {
            using (var conn = await Connection.OpenConnectionReportAsync())
            {
                var sp = @"[dbo].[TrungTamNoiDia_BCND01_BaoCaoTongHopSL]";
                var x = new
                {
                    input.Nam,
                    input.Thang,
                ...
                var res= await conn.QuerySpAsync<ReportBCND01ChiTietItem>(sp, x);
                return res;
            }
        }
        public async Task<IEnumerable<ReportBCND01ChiTietItem>> ExecuteSP2(ReportBCND01Input input)
        {
            using (var conn = await Connection.OpenConnectionReportAsync())
            {
                var sp = @"[dbo].[TrungTamNoiDia_BCND01_BaoCaoTongHopDT]";
                var x = new
                {
                    input.Nam,
                    input.Thang,
        ...
        public async Task<IEnumerable<ReportBCND01DetailItem>> ExecuteSP3(ReportBCND01Input input)
        {
            using (var conn = await Connection.OpenConnectionReportAsync())
            {
                var sp = @"[dbo].[TrungTamNoiDia_BCND01_BaoCaoTongHopDT_Detail]";
                var x = new
                {
                    input.Nam,
                    input.Thang,
                ...
                var res = await conn.QuerySpAsync<ReportBCND01DetailItem>(sp, x);
                return res;
            }
        }

    }
    public class ReportBCND01Input : IRequest<ReportBCND01Output>
    {
    ...
    public class ReportBCND01Output : BaseReportOutput<ReportBCND01Input>
    {
        public IEnumerable<ReportBCND01Item> Data { get; set; }
        public IEnumerable<ReportBCND01Item> Data2 { get; set; }
        public IEnumerable<ReportBCND01DetailItem> DataDetail { get; set; }
        public ExpandoObject Tong { get; set; }
        public ReportBCND01Output(ReportBCND01Input input) : base(input)
        {
        }
    }
    public class ReportBCND01Item
    {
        public string MaTuyen { get; set; }
        public string TenTuyen { get; set; }
        public IEnumerable<ReportBCND01ChiTietItem> Items { get; set; }
    }
    public class ReportBCND01ChiTietItem 
    {
        public string MaTuyen { get; set; }
        public string TenTuyen { get; set; }
        public string PhanLoaiXe { get; set; }
    ...
    public class ReportBCND01DetailItem
    {
        public string MaTuyen { get; set; }
        public string TenTuyen { get; set; }
        public string PhanLoaiXe { get; set; }
